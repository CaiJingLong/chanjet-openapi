...
- **Node 目标**：Node >= 22（OIDC trusted publishing 要求 + 原生 `fetch`），`engines` 字段显式声明。根 `package.json` 必须声明 `packageManager` 字段（如 `pnpm@11.21.0`），供 CI `pnpm/action-setup` 读取。pnpm 11 将 `onlyBuiltDependencies` 从 `package.json` 的 `pnpm` 字段迁移到 `pnpm-workspace.yaml` 的 `allowBuilds`。
